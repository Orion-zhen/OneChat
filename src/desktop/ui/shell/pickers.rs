...
mod tests {
    ...
    #[test]
    fn command_filter_matches_labels_and_keywords() {
        let mut delegate = CommandPaletteDelegate::new();
        delegate.filter("provider");
        assert_eq!(
            delegate.filtered,
            vec![PaletteCommand::ChooseModel, PaletteCommand::OpenSettings]
        );

        delegate.filter("conversation");
        assert!(delegate.filtered.contains(&PaletteCommand::NewConversation));
        assert!(
            delegate
                .filtered
                .contains(&PaletteCommand::FocusConversationSearch)
        );
    }

    #[test]
    fn prompt_filter_has_a_stable_empty_state() {
        let mut delegate = PromptPickerDelegate {
            all: vec![PromptPickerItem {
                name: None,
                title: "No System Prompt".into(),
                excerpt: "Continue without reusable instructions.".into(),
        ...
        delegate.filter("missing");
        assert!(delegate.filtered.is_empty());
        assert_eq!(delegate.initial_selection(), None);
    }

    #[test]
    fn prompt_excerpt_is_single_line_and_bounded() {
        let excerpt = prompt_excerpt(&format!("{}\n{}", "a".repeat(80), "b".repeat(80)));
    ...
    #[test]
    fn picker_height_grows_with_content_without_becoming_a_full_height_panel() {
        assert_eq!(picker_height(0.0), 180.0);
        assert_eq!(picker_height(168.0), 229.0);
        assert_eq!(picker_height(800.0), 420.0);
    }
}

...
mod tests {
    ...
    fn test_root() -> PathBuf {
        let root = env::temp_dir().join(format!(
            "onechat-executable-test-{}-{}",
            std::process::id(),
            crate::domain::new_id("path")
        ));
    ...
    fn resolves_windows_path_extensions() {
        let root = test_root();
        let executable = root.join("tool.CMD");
        fs::write(&executable, "@echo off\r\n").unwrap();
        let environment = ExecutionEnvironment {
            path: vec![root.clone()],
            path_ext: path_extensions(Some(OsString::from(".EXE;.CMD"))),
        };
        assert_eq!(
            environment.resolve("tool", None, &BTreeMap::new()),
            Some(executable)
        );
    ...
    fn resolves_only_executable_files() {
        ...
        assert_eq!(environment.resolve("tool", None, &BTreeMap::new()), None);
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn passes_discovered_path_to_interpreter_launchers() {
        let root = test_root();
        let launcher = root.join("launcher");
        let interpreter = root.join("onechat-test-interpreter");
        fs::write(&launcher, "#!/usr/bin/env onechat-test-interpreter\n").unwrap();
        ...
        environment.apply(&mut command, &BTreeMap::new());
    ...
    #[cfg(unix)]
    #[test]
    fn explicit_path_uses_server_cwd() {
        let root = test_root();
        ...
        fs::write(&executable, "#!/bin/sh\n").unwrap();
        fs::set_permissions(&executable, fs::Permissions::from_mode(0o755)).unwrap();
        let environment = ExecutionEnvironment { path: Vec::new() };
        assert_eq!(
            environment.resolve("bin/tool", None, &BTreeMap::new()),
            None
        );
        assert_eq!(
            environment.resolve("bin/tool", Some(&root), &BTreeMap::new()),
            Some(executable)
        );
        fs::remove_dir_all(root).unwrap();
    }
    ...
    fn configured_path_overrides_discovered_path() {
        ...
        let overrides = BTreeMap::from([(
            "PATH".to_string(),
            configured.as_os_str().to_string_lossy().into_owned(),
        )]);
        assert_eq!(
            environment.resolve("tool", None, &overrides),
            Some(executable)
        );
        fs::remove_dir_all(root).unwrap();
    }

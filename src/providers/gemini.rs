...
mod tests {
    ...
    fn request_parameters_follow_gemini_capabilities() {
        ...
        assert!(parameters.get("toolConfig").is_none());
    ...
    async fn stream_returns_tool_calls_and_sends_definitions() {
        ...
        let fixture = include_str!("../../tests/fixtures/gemini_tool_call.sse");
        let (endpoint, captured) =
            server("200 OK", "text/event-stream", fragmented(fixture, 11)).await;
        let mut request = request();
        request.provider.endpoint = endpoint;
        ...
        assert_eq!(calls[0].id, "fixture__environment");
        assert_eq!(calls[0].call_id.as_deref(), Some("call-1"));
        assert_eq!(calls[0].signature.as_deref(), Some("signed-tool-call"));

...
mod tests {
    ...
    impl TestServer {
        async fn start() -> Self {
            ...
            let exit_marker = root.join("exited");
            fs::write(
                &config_path,
                format!(
                    r#"{{
                        // Exercise direct command execution, cwd, env and JSONC.
                        "mcpServers": {{
                            "fixture": {{
                                "command": {},
                                "cwd": {},
                                "env": {{
                                    "ONECHAT_TEST_VALUE": "from-config",
                                    "ONECHAT_EXIT_MARKER": {}
                                }},
                            }},
                        }},
                    ...
                    serde_json::to_string(&exit_marker).unwrap(),
                ),
            )
            .unwrap();
            ...
            assert_eq!(snapshot.servers[0].status, McpServerStatus::Ready);
            Self {
                root,
                manager,
                exit_marker,
    ...
    async fn start_http_fixture() -> (String, CancellationToken) {
        start_http_fixture_with_headers(Vec::new()).await
    }

    async fn start_http_fixture_with_headers(
        ...
        tokio::spawn(async move {
            loop {
                ...
                tokio::spawn(handle_http_request(stream, expected_headers));
            }
        });
        (url, cancellation)
    }

    async fn handle_http_request(mut stream: TcpStream, expected_headers: Vec<(String, String)>) {
        let mut request = Vec::new();
        ...
        for (expected_name, expected_value) in expected_headers {
            assert!(
                ...
                "missing {expected_name}: {expected_value} in {headers}"
            );
        }
        ...
        if method == "DELETE" {
            write_http_response(&mut stream, "204 No Content", None).await;
            return;
        }
        ...
        let result = match message["method"].as_str().unwrap() {
            "initialize" => serde_json::json!({
                ...
                "capabilities": { "tools": {} },
                "serverInfo": { "name": "onechat-http-test", "version": "1.0.0" }
            }),
        ...
        write_http_response(&mut stream, "200 OK", Some(response.to_string())).await;
    }

    async fn start_oauth_http_fixture() -> (String, CancellationToken) {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        ...
        tokio::spawn(async move {
            loop {
                ...
                tokio::spawn(handle_oauth_http_request(stream, fixture_base_url.clone()));
            }
        });
        (base_url, cancellation)
    }

    async fn handle_oauth_http_request(mut stream: TcpStream, base_url: String) {
        let mut request = Vec::new();
        let mut chunk = [0; 4096];
        let header_end = loop {
            let read = stream.read(&mut chunk).await.unwrap();
        ...
        let headers = String::from_utf8_lossy(&request[..header_end]).into_owned();
        let request_target = headers.split_whitespace().nth(1).unwrap();
        let path = request_target.split('?').next().unwrap();
        let content_length = headers
            .lines()
            .find_map(|line| {
                let (name, value) = line.split_once(':')?;
        ...
        match path {
            ...
            | "/.well-known/oauth-protected-resource/mcp" => {
                ...
                write_http_response(&mut stream, "200 OK", Some(body.to_string())).await;
            }
            ...
            "/mcp" => {
                let authorized = headers.lines().any(|line| {
                    line.split_once(':').is_some_and(|(name, value)| {
                        name.eq_ignore_ascii_case("authorization")
                            && value.trim() == "Bearer oauth-access-token"
                    })
                });
                if !authorized {
                    let response = format!(
                ...
                let result = match message["method"].as_str().unwrap() {
                    "initialize" => serde_json::json!({
                        "protocolVersion": message["params"]["protocolVersion"],
                        "capabilities": { "tools": {} },
                        "serverInfo": { "name": "onechat-oauth-test", "version": "1.0.0" }
                    }),
                    "tools/list" => serde_json::json!({
                        "tools": [{
                            "name": "secure_echo",
                    ...
                    method => panic!("unexpected OAuth MCP method: {method}"),
                };
                let response = serde_json::json!({ "jsonrpc": "2.0", "id": id, "result": result });
                write_http_response(&mut stream, "200 OK", Some(response.to_string())).await;
            }
            path => panic!("unexpected OAuth fixture path: {path}"),
        }
    }

    async fn write_http_response(stream: &mut TcpStream, status: &str, body: Option<String>) {
    ...
    #[tokio::test]
    async fn stdio_server_lists_and_calls_tools_with_configured_environment() {
        let server = TestServer::start().await;
        let snapshot = server.manager.snapshot().await;
        assert_eq!(snapshot.servers[0].tools.len(), 2);
        assert_eq!(snapshot.servers[0].tools[0].name, "environment");
        assert!(snapshot.servers[0].implementation.is_some());

        let tools = server.manager.tools().await;
        assert_eq!(tools.len(), 2);
        assert_eq!(tools[0].name, "fixture__environment");
        assert_eq!(tools[0].input_schema["type"], "object");

        let result = server
            .manager
            .call_model_tool("fixture__environment", Map::new(), CancellationToken::new())
            .await
            .unwrap();
        let value = serde_json::to_string(&result).unwrap();
        assert!(value.contains("from-config"));
        assert!(value.contains(server.root.to_string_lossy().as_ref()));

        server.manager.shutdown().await;
        assert!(server.exit_marker.exists());
    }

    #[tokio::test]
    async fn streamable_http_server_lists_and_calls_tools() {
        let (url, fixture) = start_http_fixture().await;
        let root = env::temp_dir().join(format!(
            "onechat-http-mcp-test-{}-{}",
            std::process::id(),
            crate::domain::new_id("server")
        ));
        fs::create_dir_all(&root).unwrap();
        ...
        assert_eq!(snapshot.servers[0].status, McpServerStatus::Ready);
        assert!(matches!(
            &snapshot.servers[0].transport,
            McpServerTransportSnapshot::Http { url: snapshot_url } if snapshot_url == &url
        ));
        assert!(snapshot.servers[0].enabled);
        assert_eq!(snapshot.servers[0].tools[0].name, "echo");
        assert!(snapshot.servers[0].tools[0].enabled);
        let result = manager
            .call_tool("remote", "echo", Map::new(), CancellationToken::new())
            .await
            .unwrap();
        assert!(
        ...
        manager.test_server("remote".into()).await.unwrap();

        let snapshot = manager
            .set_tool_enabled("remote".into(), "echo".into(), false)
            .await
            .unwrap();
        assert!(!snapshot.servers[0].tools[0].enabled);
        ...
        assert!(!all_tools[0].enabled);
        assert!(
            manager
                .call_model_tool("remote__echo", Map::new(), CancellationToken::new())
                .await
                .is_err()
        );
        manager
            .set_tool_enabled("remote".into(), "echo".into(), true)
            .await
            .unwrap();
        assert_eq!(manager.tools().await.len(), 1);

        let snapshot = manager
            .set_server_enabled("remote".into(), false)
            .await
            .unwrap();
        assert!(!snapshot.servers[0].enabled);
        assert_eq!(snapshot.servers[0].status, McpServerStatus::Disabled);
        assert!(manager.tools().await.is_empty());
        manager.test_server("remote".into()).await.unwrap();
        let snapshot = manager
            .set_server_enabled("remote".into(), true)
            .await
            .unwrap();
        assert!(snapshot.servers[0].enabled);
        assert_eq!(snapshot.servers[0].status, McpServerStatus::Ready);
    ...
    #[tokio::test]
    async fn streamable_http_sends_custom_headers_and_bearer_token() {
        let (url, fixture) = start_http_fixture_with_headers(vec![
            ("x-api-key".into(), "secret".into()),
            ("authorization".into(), "Bearer token".into()),
        ])
        .await;
        let root = env::temp_dir().join(format!(
            "onechat-authenticated-http-mcp-test-{}-{}",
            std::process::id(),
            crate::domain::new_id("server")
        ));
        fs::create_dir_all(&root).unwrap();
    ...
    #[tokio::test]
    async fn streamable_http_uses_configured_proxy() {
        let (proxy_endpoint, fixture) = start_http_fixture().await;
        let proxy = proxy_endpoint.trim_end_matches("/mcp");
        let root = env::temp_dir().join(format!(
            "onechat-proxied-http-mcp-test-{}-{}",
            std::process::id(),
            crate::domain::new_id("server")
        ));
        fs::create_dir_all(&root).unwrap();
    ...
    #[tokio::test]
    async fn oauth_client_credentials_authenticates_http_server() {
        let (base_url, fixture) = start_oauth_http_fixture().await;
        let root = env::temp_dir().join(format!(
            "onechat-client-credentials-mcp-test-{}-{}",
            std::process::id(),
            crate::domain::new_id("server")
        ));
        fs::create_dir_all(&root).unwrap();
        ...
        let snapshot = manager.reload().await;
        assert_eq!(snapshot.servers[0].status, McpServerStatus::Ready);
        assert_eq!(snapshot.servers[0].tools[0].name, "secure_echo");
    ...
    #[tokio::test]
    async fn authorization_code_oauth_persists_and_reuses_credentials() {
        let (base_url, fixture) = start_oauth_http_fixture().await;
        let root = env::temp_dir().join(format!(
            "onechat-authorization-code-mcp-test-{}-{}",
            std::process::id(),
            crate::domain::new_id("server")
        ));
        fs::create_dir_all(&root).unwrap();
        ...
        .unwrap();
        let manager = std::sync::Arc::new(McpManager::new(config_path));
        let snapshot = manager.reload().await;
        assert_eq!(
        ...
        let authorizing = {
            ...
            tokio::spawn(async move { manager.authorize_server("secure".into(), url_sender).await })
        };
        let authorization_url = reqwest::Url::parse(&url_receiver.recv().await.unwrap()).unwrap();
        ...
        let snapshot = authorizing.await.unwrap().unwrap();
        assert_eq!(snapshot.servers[0].status, McpServerStatus::Ready);
        let cache_dir = oauth_cache_dir(manager.config_path());
        assert!(cache_dir.is_dir());
        #[cfg(unix)]
        {
            let token_file = fs::read_dir(&cache_dir)
                .unwrap()
                .next()
                .unwrap()
                .unwrap()
    ...
    #[tokio::test]
    async fn tool_calls_are_cancellable_without_stopping_the_server() {
        let server = TestServer::start().await;
        let cancellation = CancellationToken::new();
        ...
        let error = server
            .manager
            .call_tool("fixture", "slow", Map::new(), cancellation)
            .await
            .unwrap_err();
        assert_eq!(error.to_string(), "MCP tool call cancelled");
        assert!(started.elapsed() < Duration::from_secs(1));
        server.manager.shutdown().await;
    }

    #[tokio::test]
    async fn invalid_reload_keeps_existing_sessions_alive() {
        let server = TestServer::start().await;
        fs::write(server.manager.config_path(), "{ broken").unwrap();
        let snapshot = server.manager.reload().await;
    ...
    #[test]
    fn model_tool_names_are_provider_safe_and_bounded() {
        assert_eq!(
            model_tool_name("filesystem", "read_file"),
            "filesystem__read_file"
        );
        let transformed = model_tool_name("123 server", &"工具".repeat(40));
        assert!(transformed.len() <= 64);
        assert!(
            transformed
                .chars()
                .all(|character| character.is_ascii_alphanumeric() || character == '_')
    ...
    #[test]
    fn absolute_executables_are_resolved_without_a_shell() {
        let executable = env::current_exe().unwrap();
        let environment = ExecutionEnvironment::inherited();
        assert_eq!(
            environment.resolve(executable.to_str().unwrap(), None, &BTreeMap::new()),
            Some(executable)
        );
        assert!(
            environment
                .resolve(
                    "onechat-command-that-does-not-exist",
                    None,
                    &BTreeMap::new()
                )
                .is_none()

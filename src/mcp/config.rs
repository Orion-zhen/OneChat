...
mod tests {
    ...
    fn parse(source: &str) -> Result<McpConfig> {
        McpConfig::parse(source)
    }

    fn temp_config(source: &str) -> PathBuf {
        let path = std::env::temp_dir().join(format!(
            "onechat-mcp-config-{}-{}.jsonc",
            std::process::id(),
            crate::domain::new_id("test")
        ));
    ...
    #[test]
    fn config_accepts_jsonc_defaults_and_stdio_fields() {
        let config = parse(
            r#"{
                // npm and uv programs are resolved from OneChat's PATH.
                "mcpServers": {
                    "filesystem": {
                        "command": " npx ",
                        "args": ["-y", "server"],
        ...
        let McpServerConfig::Stdio(filesystem) = &config.servers["filesystem"] else {
            panic!("expected stdio server");
        };
        assert!(filesystem.enabled);
        assert_eq!(filesystem.command, "npx");
        assert_eq!(filesystem.args, ["-y", "server"]);
        assert_eq!(filesystem.env["TOKEN"], "secret");
        assert_eq!(filesystem.cwd.as_deref(), Some(Path::new("/tmp")));
        assert!(!config.servers["disabled"].enabled());
    }

    #[test]
    fn config_accepts_streamable_http_servers() {
        let config = parse(
            r#"{
                "mcpServers": {
                    "exa": { "url": " https://mcp.exa.ai/mcp " },
                    "disabled": { "enabled": false, "url": "http://localhost:3000/mcp" }
                }
            }"#,
        )
        .unwrap();

        let McpServerConfig::Http(exa) = &config.servers["exa"] else {
    ...
    #[test]
    fn config_accepts_http_authentication_headers_and_proxy() {
        let config = parse(
            r#"{
                "mcpServers": {
                    "api-key": {
        ...
        let McpServerConfig::Http(api_key) = &config.servers["api-key"] else {
            panic!("expected HTTP server");
        };
        assert_eq!(api_key.headers["X-API-Key"], "secret");
        assert_eq!(api_key.proxy.as_deref(), Some("socks5://127.0.0.1:1080"));
        assert_eq!(
            config.servers["bearer"],
        ...
        let McpServerConfig::Http(browser) = &config.servers["browser"] else {
            panic!("expected HTTP server");
        };
        let browser = browser.oauth.as_ref().unwrap();
        assert_eq!(browser.flow, McpOAuthFlow::AuthorizationCode);
        assert_eq!(browser.callback_port, Some(9876));
        let McpServerConfig::Http(machine) = &config.servers["machine"] else {
            panic!("expected HTTP server");
        };
        assert_eq!(
            machine.oauth.as_ref().unwrap().flow,
            McpOAuthFlow::ClientCredentials
        );
    }

    #[test]
    fn config_rejects_conflicting_or_invalid_http_authentication() {
        assert!(
            parse(
                r#"{ "mcpServers": { "bad": {
                    "url": "https://example.com/mcp",
        ...
        assert!(
            parse(
                r#"{ "mcpServers": { "bad": {
                    "url": "https://example.com/mcp",
                    "oauth": { "flow": "clientCredentials", "clientId": "client" }
                } } }"#
            )
            .is_err()
        );
    ...
    #[test]
    fn config_rejects_invalid_transports_working_directories_and_typos() {
        assert!(parse(r#"{ "mcpServers": { "bad": {} } }"#).is_err());
        assert!(parse(r#"{ "mcpServers": { "bad": { "url": "ftp://example.com" } } }"#).is_err());
        assert!(
            parse(
                r#"{ "mcpServers": { "bad": { "url": "https://example.com", "command": "uv" } } }"#
            )
            .is_err()
        );
        assert!(parse(r#"{ "mcpServers": { "bad": { "command": "uv", "cwd": "." } } }"#).is_err());
        assert!(
            parse(r#"{ "mcpServers": { "bad": { "command": "uv", "arguments": [] } } }"#).is_err()
        );
    }

    #[test]
    fn cst_updates_preserve_existing_comments_and_unrelated_formatting() {
...
"#,
        ...
        McpConfig::upsert(
            ...
            McpServerConfig::Stdio(McpStdioServerConfig {
                command: "uvx".into(),
                args: vec!["mcp-server-fetch".into(), "--verbose".into()],
                env: BTreeMap::from([("TOKEN".into(), "new".into())]),
                ..Default::default()
            }),
        )
        .unwrap();
        ...
        assert!(output.contains("// fetch server comment"));
        assert!(output.contains("// command comment"));
        assert!(output.contains("// first argument comment"));
        assert!(output.contains("// token comment"));
        assert!(output.contains("// unrelated comment"));
        assert!(output.contains("\"other-server\": { \"command\": \"keep\" }"));
        let config = McpConfig::load(&path).unwrap();
        let McpServerConfig::Stdio(fetch) = &config.servers["fetch"] else {
            panic!("expected stdio server");
        };
    ...
    #[test]
    fn import_merges_servers_without_rewriting_comments() {
        let path = temp_config(
            r#"{
  // keep this comment
  "mcpServers": {
...
"#,
        ...
        assert!(config.servers.contains_key("existing"));
        let McpServerConfig::Stdio(fetch) = &config.servers["fetch"] else {
            panic!("expected stdio server");
        };
        assert_eq!(fetch.args, ["mcp-server-fetch"]);
        fs::remove_file(path).unwrap();
    }

    #[test]
    fn server_and_tool_toggles_update_only_the_relevant_config() {
        let path = temp_config(
            r#"{
  "mcpServers": {
    // Keep server comments while toggling.
    "exa": { "url": "https://mcp.exa.ai/mcp" }
  }
}"#,
        );

        McpConfig::set_server_enabled(&path, "exa", false).unwrap();
        McpConfig::set_tool_enabled(&path, "exa", "web_search_exa", false).unwrap();
        let config = McpConfig::load(&path).unwrap();
        assert!(!config.servers["exa"].enabled());
        assert!(
            config.servers["exa"]
                .disabled_tools()
        ...
        assert!(
            fs::read_to_string(&path)
                .unwrap()
                .contains("Keep server comments")
        );

        McpConfig::set_tool_enabled(&path, "exa", "web_search_exa", true).unwrap();
        assert!(
            McpConfig::load(&path).unwrap().servers["exa"]
                .disabled_tools()
                .is_empty()
        );
        fs::remove_file(path).unwrap();
    ...
    #[test]
    fn import_and_upsert_preserve_http_shape() {
        let path = temp_config(r#"{ "mcpServers": {} }"#);
        McpConfig::import(
            &path,
            r#"{ "mcpServers": { "exa": { "url": "https://mcp.exa.ai/mcp" } } }"#,
        )
        .unwrap();
        ...
        let output = fs::read_to_string(&path).unwrap();
        assert!(output.contains("https://mcp.exa.ai/mcp"));
        assert!(output.contains("X-API-Key"));
        assert!(output.contains("callbackPort"));
        assert!(!output.contains("command"));
        let config = McpConfig::load(&path).unwrap();
        let McpServerConfig::Http(exa) = &config.servers["exa"] else {
            panic!("expected HTTP server");
        };
        assert_eq!(exa.oauth.as_ref().unwrap().callback_port, Some(9876));
        fs::remove_file(path).unwrap();
    }
}
